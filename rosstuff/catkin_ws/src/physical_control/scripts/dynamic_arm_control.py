...
class RobotControl():
    ...
    def set_coordinates(self, targetCoords):
        #TODO:
        ...
        while (phi_3 < upper):
            z2 = z - L3 * np.sin(phi_3)
            y2 = y - L3 * np.cos(phi_3)
            ...
            c_theta2 = (z2**2 + y2**2 -L1**2 - L2**2)/(2* L1 *L2)#TODO: Verify
            if(c_theta2 > 1):
                phi_3+= increment
                solutionFlag = 0
                continue
        ...
        if(solutionFlag == 0):
            ...
            while (phi_3 > lower):
                ...
                c_theta2 = (z2**2 + y2**2 -L1**2 - L2**2)/(2* L1 *L2)
                if(c_theta2 > 1):
                    phi_3-= increment
                    solutionFlag = 0
                    continue
                theta_2 = np.arctan2(c_theta2, np.sqrt(1 - c_theta2**2))
                theta_1 = np.arctan2(y2,z2) - \
                    np.arctan2(L1 + L2 * np.cos(theta_2), L2 * np.sin(theta_2))
                theta_3 = phi_3 - theta_2 - theta_1
                solutionFlag = 1
                if ((theta_3 < -2.1) or (theta_3 > 2)\
                        or (theta_2 < -2.5) or (theta_2 > 2)):
